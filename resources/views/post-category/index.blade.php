...
@section('title')
    ...
    <a href="{{ route('post-categories.create', ['category_id' => request()->segment(3)]) }}"
       class="btn btn-sm btn-primary">Create</a>
    {{--    @endif--}}
@stop

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('post-categories.index') }}">Root Categories</a></li>
    @if ($rootCategory->id)
        <li class="breadcrumb-item"><a
                href="{{ route('post-categories.index', $rootCategory->parent_category_id) }}">{{ $rootCategory->name }}</a>
        </li>
    @endif
@stop
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach ($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('post-categories.index', $category->id) }}"><i
                                ...
                                <td>
                                    ...
                                    @if ($category->posts_count === 0)
                                        ...
                                        <form id="delete-category-{{ $category->id }}"
                                              action="{{ route('post-categories.destroy', $category->id) }}"
                                              method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                ...
                                {{--                                <td>--}}
                                {{--                                    <a class="" href="{{ route('posts.index', ['category' => $category->id]) }}">Show--}}
                                {{--                                        all posts</a>--}}
                                {{--                                </td>--}}
                            </tr>
                        @endforeach
