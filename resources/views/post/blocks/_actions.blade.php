<button class="btn btn-sm bg-primary d-inline">
    <a href="{{ route('posts.edit', $post->id) }}"><em class="fas fa-eye"></em> Edit </a>
</button>
...
@if ($post->isPublished() && !$post->is_trending_now)
    <form id="post-top-{{ $post->id }}" action="{{ route('posts.update.main', $post->id) }}" method="POST"
          class="d-inline">
        @csrf
        @method('put')
    </form>
@endif
<form id="post-destroy-{{ $post->id }}" action="{{ route('posts.destroy', $post->id) }}" method="post">
    @csrf
    @method('delete')
</form>
