...
@section('title')
    Tag
    <a href="{{ route('alphanews.tags.create', ['tag_id' => request()->segment(3)]) }}" class="btn btn-sm btn-primary">Create</a>
@stop
                            ...
                            @foreach($tags as $tag)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form id="delete-tag-{{ $tag->id }}" action="{{ route('alphanews.tags.destroy', $tag->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
